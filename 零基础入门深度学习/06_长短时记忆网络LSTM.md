...
如果内容也用 Sigmoid，那 $\tilde{\mathbf{c}}_t$ 永远非负——**网络就永远无法写入负内容**（记忆只能靠遗忘门衰减变小，压不成负值）。
...
四个门各需要一套 $\mathbf{W}\in\mathbb{R}^{n\times(d+n)}$ 和 $\mathbf{b}\in\mathbb{R}^n$：
...
- **把两者的循环权重谱半径都控制为 0.9**（LSTM 为四个门的循环块；这样才是公平比较——否则可能一方是"梯度爆炸"而非"梯度消失"）；
- 在最后一步注入梯度（每分量取 1，表中报告 $\lVert\cdot\rVert_2/\sqrt{2n}$ 归一后的范数），看它回传到各步时还剩多少。
...
而每步衰减率的上界是 $\gamma_{\max}\cdot\rho$，其中 $\gamma_{\max}=\max|\tanh'|\le1$、$\rho=0.9$。实测有效衰减因子 $0.46/0.9\approx0.51$，在上界之内、**与 tanh 在常用取值范围内的导数（约 0.5）吻合**。理论、实测互相印证。
...
> 在这个"记住 45 步前一个脉冲"的合成任务（序列长 50 天，脉冲出现在第 5 天）上，**如果只给 16 个隐单元和有限的训练轮数，LSTM 同样学不动 30 步以上的滞后**（约 25 步以内的滞后通常能学会）——它的优势体现在**梯度流**上（如 §4.2 实测），而不是"包治百病"。
...
| 训练轮数/数据量 | 需要更多样本与迭代 |
| 任务本身的难度 | "记住一个孤立脉冲 45 步"是**很难**的任务，即使对人也是 |
...
class LSTM:
    ...
    def forward(self, X):
        ...
        for t in range(T):
            z = np.concatenate([X[:, t], self.h[:, t]], axis=1)   # 拼接 x_t 与 h_{t-1}
            self.z[:, t] = z
...
**示例运行结果**（梯度检查偏差的末位随 NumPy/平台略有出入，量级不变）：
...
4. 数学上，$\partial\mathbf{c}_t/\partial\mathbf{c}_{t-1}$ 的主干项是 $\mathrm{diag}(\mathbf{f}_t)$，**不含权重矩阵**，这就是传送带；
...
**3.** 恒为 0（完全不保留旧记忆，每步都清空）→ 长期记忆通道被切断，退化成一个无细胞记忆的门控 RNN（历史仍可通过 $\mathbf{h}_{t-1}$ 影响各门，但长期记忆完全消失）。
恒为 1（完全保留，什么都不忘）→ 记忆 $\mathbf{c}_t$ 只累加不遗忘（增量本身可正可负，但噪声也丢不掉），模型失去"遗忘"能力——实践中通常很快就发散或饱和。
...
> 📌 **入门版与深入版的关系**：本篇是**入门版**——讲清"是什么、为什么、怎么算"，配手算与可运行的 NumPy 实现；案例1 那篇是**深入版**——严格的 BPTT 全推导、与 PyTorch 实现的对齐、以及在单点位移预测上的完整工程实践。**建议先读完本篇，再读那篇**。
