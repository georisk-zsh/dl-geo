...
```text
零基础入门深度学习（本模块）
  01 感知器 ──→ 02 线性单元与梯度下降 ──→ 03 神经网络与反向传播
                                              │
                        ┌─────────────────────┼─────────────────────┐
                        ↓                     ↓                     ↓
                   04 卷积神经网络        05 循环神经网络        06 LSTM
                        │                     │                     │
                        ↓                     ↓                     ↓
             案例3 CNN-Transformer      案例1 LSTM          案例1 LSTM（深入版）
              原理详解 §2 因果卷积        原理详解 §2–§3        原理详解 §4–§8
```
...
> 📐 **数学符号看不懂**？本模块每篇都假定你只学过高中数学。遇到矩阵、偏导数、链式法则等符号，随时查阅 [算法原理_数学预备知识.md](../算法原理_数学预备知识.md)——那份文档用大白话 + 手算例子讲清了全部所需工具。
