...
> 🎓 **完全零基础请从这里开始**：[**零基础入门深度学习/**](零基础入门深度学习/README.md)——6 篇渐进式教程（感知器 → 线性单元与梯度下降 → 神经网络与反向传播 → 卷积神经网络 → 循环神经网络 → LSTM），每篇配结构图、手算例子与**可运行的 NumPy 从零实现**，学完即具备读下面四篇原理详解的全部前置知识。
>
...
| 资源 | 内容 |
|---|---|
| [**零基础入门深度学习/**](零基础入门深度学习/README.md)　前置模块 | 6 篇从零开始的教程：感知器 / 线性单元与梯度下降 / 神经网络与反向传播 / 卷积神经网络 / 循环神经网络 / LSTM。每篇含 Mermaid 结构图、手算数值例子、**可独立运行的 NumPy 实现**与梯度检查、自检题（折叠答案） |
