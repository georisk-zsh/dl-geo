...
- [二、学习路径](#二学习路径)
- [三、课程内容](#三课程内容)
- [四、快速开始（零基础请逐步照做）](#四快速开始零基础请逐步照做)
- [五、零基础入门深度学习](#五零基础入门深度学习)
- [六、算法案例](#六算法案例)
...
| | |
|---|---|
| **6 篇零基础教程** | [零基础入门深度学习/](零基础入门深度学习/README.md)：从"什么是神经元"一路讲到 LSTM，每篇配可运行的 NumPy 代码 |
| **4 篇算法原理详解** | LSTM / GCN / CNN-Transformer / XGBoost+SHAP 的完整数学推导、结构图与数值算例 |
| **1 份数学工具包** | [数学预备知识.md](零基础入门深度学习/数学预备知识.md)：矩阵、偏导、链式法则、softmax、特征值的大白话 + 手算版 |
| **4 个算法案例** | 每个含一份**原理 PPT** + 一本**可执行 Notebook**（已执行、带输出） |
| **1 条完整方法链** | 从零实现 → 精度检验 → 调参 → 物理融合改进 → 不确定性量化 |
| **2 个拓展案例** | 论文复现、经典时序基线对照 |
| **1 份配套教程** | PostgreSQL 从零开始（监测数据的建库、查询与运维） |
| **1 套教学配套** | [教学配套/](教学配套/)：教学大纲（48 学时）+ 4 次平时作业 + 文献研读 + 大作业任务书（均含评分标准） |
| **1 篇数据准备实战** | [数据准备篇/](数据准备篇/)：审计 → 清洗（缺失/野值/台阶/漂移）→ 防泄漏划分 → 滑窗张量化，配 8 处污染的练习数据 |

**适合谁**：

- **完全零基础**（没学过神经网络、没写过深度学习代码）→ 从 [零基础入门深度学习/](零基础入门深度学习/README.md) 开始，6 篇教程会把前置知识补齐；
...
| 层 | 材料 | 适合谁 | 学完得到什么 |
|---|---|---|---|
| ① **前置基础** | [零基础入门深度学习/](零基础入门深度学习/README.md)（6 篇） | 完全没学过神经网络 | 手推感知器、梯度下降、反向传播；从零实现 CNN/RNN/LSTM |
| ② **数学工具** | [数学预备知识.md](零基础入门深度学习/数学预备知识.md)（18 节） | 读文档遇到数学符号卡住时随时查 | 矩阵乘法、逐元素乘与 diag、偏导数梯度、链式法则、softmax、泰勒展开、特征值…… |
...
```mermaid
flowchart TB
  L1["① 零基础入门深度学习 · 6 篇<br/>01 感知器 → 02 梯度下降 → 03 反向传播<br/>04 卷积神经网络 → 05 循环神经网络 → 06 LSTM"]
...
1. [零基础入门深度学习/](零基础入门深度学习/README.md) 的 6 篇，**每篇的代码都亲手敲一遍**（约 15 小时）；
2. 按兴趣选一个算法案例，先读该案例的**原理详解**，再跑对应的 **Notebook**；
3. 遇到看不动的数学符号 → 查 [数学预备知识](零基础入门深度学习/数学预备知识.md) 的对应小节；
...
> 🎓 **完全零基础请先学**：[五、零基础入门深度学习](#五零基础入门深度学习)——6 篇渐进式教程，学完即具备读四篇原理详解的全部前置知识。
>
> 📐 **零基础读者先看**：[数学预备知识.md](零基础入门深度学习/数学预备知识.md)——矩阵、偏导数、链式法则、softmax、特征值等工具的「大白话 + 手算例子」版本，四篇原理详解共用。
>
...
📂 [零基础入门深度学习/](零基础入门深度学习/README.md)　·　本模块只需 NumPy + Matplotlib，刻意不用 PyTorch——从零手写才能真懂。

| 顺序 | 文档 | 核心问题 | 学完你能 |
|---|---|---|---|
| 1 | [01_感知器](零基础入门深度学习/01_感知器.md) | 一个"神经元"怎么工作？ | 手推感知器学习规则，写出二分类程序 |
| 2 | [02_线性单元和梯度下降](零基础入门深度学习/02_线性单元和梯度下降.md) | 怎么让模型**自己**找到好参数？ | 手推平方误差的梯度，实现梯度下降做回归 |
| 3 | [03_神经网络和反向传播算法](零基础入门深度学习/03_神经网络和反向传播算法.md) | 多层网络怎么训练？ | 手推 BP 算法，从零实现一个能解 XOR 的网络 |
| 4 | [04_卷积神经网络](零基础入门深度学习/04_卷积神经网络.md) | 怎么处理图像/网格数据？ | 手算二维卷积，从零实现一个迷你 CNN |
| 5 | [05_循环神经网络](零基础入门深度学习/05_循环神经网络.md) | 怎么处理序列数据？ | 手推 BPTT，从零实现 RNN 并理解梯度消失 |
| 6 | [06_长短时记忆网络LSTM](零基础入门深度学习/06_长短时记忆网络LSTM.md) | 怎么让网络记住长期信息？ | 手推 LSTM 前向与反向，从零实现 LSTM |

**学完如何衔接**：04 卷积神经网络 → [案例3 CNN-Transformer](#案例3-cnn-transformer-混合架构)；05 循环神经网络 / 06 LSTM → [案例1 LSTM](#案例1-lstm-单点位移时序预测)。数学符号看不懂时，随时查阅 [数学预备知识.md](零基础入门深度学习/数学预备知识.md)。
...
| 资源 | 内容 |
|---|---|
| [**教学配套/**](教学配套/)　教学材料 | [教学大纲](教学配套/教学大纲.md)（16 周 × 3 学时，目标—考核对应表）+ [平时作业](教学配套/平时作业.md)（4 次 + 文献研读，全部带评分标准）+ [大作业任务书](教学配套/大作业任务书.md)（方法链七步 + 报告红线 + 评分锚点） |
| [**数据准备篇/**](数据准备篇/)　数据实战 | 从原始监测 CSV 到模型输入张量的全流程：系统审计、时间轴修复、缺失/野值/台阶/单位漂移四类脏数据的检测与修复、防泄漏划分与标准化、滑窗张量化；配 8 处植入污染的练习数据（真值在附录） |
