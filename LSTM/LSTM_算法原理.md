...
> **[📐 数学预备知识](../深度学习算法入门/数学预备知识.md)**：矩阵、导数、链式法则、softmax、特征值……零基础先读这一份（含手算例子）
...
把 $\mathrm{diag}(\mathbf{v})\,\mathbf{u}=\mathbf{v}\odot\mathbf{u}$（见[数学预备知识 §3.1](../深度学习算法入门/数学预备知识.md)）代入，即得上面的递推式。**注意路径 1 里带着 $\tanh'$ 与 $\mathbf{o}_t$（都会压缩梯度），路径 2 里只有 $\mathbf{f}$（可学到接近 1）——这就是为什么长程梯度主要走路径 2。**
